...
# Emscripten build. OpenMalaria doesn't parse XML by hand: xsd/xsdcxx
...
#                      embedded (never fetched by URL): dead code, not a
#                      feature we're giving up.
...
#                      external locale file at runtime -- consistent with the
#                      "zero external assets" goal for the final module.
# threads=OFF       -- this build is deliberately single-threaded (real wasm
#                      threads need SharedArrayBuffer, which needs COOP/COEP
#                      response headers -- a hosting requirement we want to
#                      avoid). Exactly one scenario is ever parsed per module
#                      instance, so thread-safety is moot.
...
#                      link -- a runtime-only bug, not a link error.
