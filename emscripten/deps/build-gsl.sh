...
# vector transmission matrix math
set -euo pipefail
...
if [ ! -f "${WORK_DIR}/${GSL_TARBALL}" ]; then
  ...
  curl -fsSL --retry 5 --retry-delay 3 --retry-all-errors -o "${WORK_DIR}/${GSL_TARBALL}" "${GSL_URL}"
fi
...
# This is a generic autotools-cross-compilation issue (test with
...
# to run them on, so building them at all is pointless
