...
#
# Design: the repo's CMakeLists.txt is left untouched apart from the one
# pthread guard (see git history) -- its hand-rolled cmake/Find*.cmake
# modules do plain hardcoded-path find_library/find_path calls with no wasm
# awareness. Rather than patch them, this script pre-seeds the exact cache
...
#
# xsd/xsdcxx is a code generator, not a runtime library: given a .xsd it
# emits ordinary, portable C++ that itself calls the Xerces-C API, once, at
# build time. That generated C++ then gets compiled to wasm along with
# everything else -- the generator tool itself never runs in the browser.
# So we want the fast native xsd binary, pointed at wasm-built Xerces for the
# code it emits to link against -- not a cross-compiled xsd.
...
# wasm-built Xerces archive -- an ABI mismatch surfacing as confusing link
...
# Deliberate choice: embed, don't fetch. The resulting module has zero
# runtime network dependency and works identically in Node, an offline
...
# Empirical within-host model -- unlike a native CLI invocation, a browser
# caller can't pre-declare which model a generated scenario will use, and
# the file is small enough that always-embedding beats a runtime surprise.
...
# Linker flags -- this is the part that actually shapes the JS-facing API:
#
...
#   the mechanism that makes "spin up a fresh wasm instance per run" possible
#   at all. OpenMalaria has global static state (sim, master_RNG,
...
#   in-memory filesystem -- zero new C++ surface, byte-identical behavior to
#   the native binary (verifiable by diffing output), stays in sync with
#   upstream for free. Trade-off: a whole-XML-in/whole-TSV-out API, no
#   fine-grained/streaming calls -- an acceptable fit given the CLI is
...
#   readFile directly -- a usage the linker can't see from the C++ side.
#
...
#
# -sSTACK_SIZE=1MB
#   Bumped from the default: a C++ simulator has nontrivial call depth
...
#
# -sASSERTIONS=1
#   Emscripten's runtime sanity checks and clearer failure messages. Worth
#   having on during bring-up; a legitimate later lever to strip for a
#   smaller production build once things are stable.
#
# -fexceptions
...
#   OpenMalaria's own util/errors.h machinery rely on real C++ exceptions
